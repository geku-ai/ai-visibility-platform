  ...
  private async getProviderConfig(provider: string): Promise<LLMConfig> {
    switch (provider) {
      ...
      case 'anthropic':
        return {
          provider: 'anthropic',
          model: process.env.ANTHROPIC_MODEL || 'claude-3-opus-20240229', // REVERTED: Original working model from Nov 21
      ...
      case 'gemini':
        return {
          provider: 'gemini',
          model: process.env.GEMINI_MODEL || 'gemini-pro', // REVERTED: Original working model from Nov 21
